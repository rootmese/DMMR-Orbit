...
#include "session_connection.h" //como estão na mesma pasta e um é uma extenção do outro, aí uso aspas
...
static void session_manager_enqueue(struct circle_buffer *cb, struct node_circle_buffer* n){
    ...
    if(n)
        pool = upsert_session(n, n->n.port);
...
struct dmmr_session_connection_manager* new_session_connection_manager(struct circle_buffer* cb, struct dmmr_scheduler* sched, struct cfg_server_server *css) {
    if (!cb || !sched)
        return 0;
